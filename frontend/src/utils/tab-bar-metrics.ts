 ...
 * de reconstituer l'arithmétique `SUNSET_BAR_MARGIN + SUNSET_BAR_HEIGHT`/
 * `CLASSIC_BAR_HEIGHT` à la main.
 *
 * L'inset bas ne s'ajoute QUE pour Classique : sa barre est en flux normal
 * (pas de `position:absolute`), donc `insets.bottom` agrandit réellement sa
 * boîte (voir `_layout.tsx`). La pilule Sunset garde une marge fixe
 * (`SUNSET_BAR_MARGIN`) quel que soit l'inset — l'ajouter ne fait que
