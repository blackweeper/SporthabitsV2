...
/** Purges every cached `Plan` generated by `findOrCreateProgramPlan` for the
 * given program ids, so the next visit to any of their days rebuilds the
 * plan from the current `Program` definition instead of serving a stale one
 * — `findOrCreateProgramPlan` otherwise reuses a plan forever once created,
 * even after the source program data has been corrected. Safe: `Plan` here
 * only holds the day's exercise list, never logged results (those live in
 * `WorkoutSession`) or completion state (`ActiveProgram.completedSessions`),
 * so nothing the user has already done is lost. */
export async function deleteProgramPlansForIds(programIds: string[]): Promise<void> {
  const raw = await AsyncStorage.getItem(PLANS_KEY);
  if (!raw) return;
  const list: Plan[] = JSON.parse(raw);
  const kept = list.filter((p) => !programIds.includes(p.programSource?.programId ?? ''));
  if (kept.length !== list.length) {
    await AsyncStorage.setItem(PLANS_KEY, JSON.stringify(kept));
  }
}
